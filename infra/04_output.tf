...
output "ssh_jump" {
  value       = "ssh -i keys/${local.project_name}.pem ec2-user@${aws_eip.jump.public_ip}"
  description = "SSH from workstation to jump host."
}

# Laptop -> lb / app VMs (through jump via ProxyJump)
output "ssh_lb" {
  value       = "ssh -i keys/${local.project_name}.pem -J ec2-user@${aws_eip.jump.public_ip} ec2-user@${aws_instance.lb.private_ip}"
  description = "SSH from workstation to lb via jump (ProxyJump)."
}

output "ssh_app_vm1" {
  value       = "ssh -i keys/${local.project_name}.pem -J ec2-user@${aws_eip.jump.public_ip} ec2-user@${aws_instance.app_vm1.private_ip}"
  description = "SSH from workstation to app-vm1 (canary) via jump (ProxyJump)."
}

output "ssh_app_vm2" {
  value       = "ssh -i keys/${local.project_name}.pem -J ec2-user@${aws_eip.jump.public_ip} ec2-user@${aws_instance.app_vm2.private_ip}"
...
output "jenkins_tunnel" {
  value       = "ssh -i keys/${local.project_name}.pem -L 8080:localhost:8080 ec2-user@${aws_eip.jump.public_ip}"
